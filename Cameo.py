...
def predict(test):
    ...
    maps = mapping[label]
    print("CONFIDENCE SCORE: " + str(conf)+'%')

    draw_rect(img, rect)
    draw_text(img, maps, rect[0], rect[1])
    draw_text(img, str(round(conf, 2)), rect[-1], rect[-0])
    return img


print("Predicting....")

sample1 = cv2.imread("C:\\Users\\HP\\PycharmProjects\\Rishabh\\Database(Custom)\\Testing_faces\\s4\\12.jpg")
